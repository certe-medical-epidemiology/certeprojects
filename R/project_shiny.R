...
#' @export
#' @importFrom shiny fluidPage sidebarLayout sidebarPanel textInput textAreaInput uiOutput selectInput checkboxInput br p hr actionButton radioButtons renderUI tagList selectizeInput dateInput observeEvent updateTextInput runGadget stopApp dialogViewer incProgress withProgress tags icon mainPanel img a updateCheckboxInput updateRadioButtons
...
project_add <- function(board = read_secret("trello.default.board"),
  ...
  ui <- fluidPage(
    ...
    sidebarLayout(
      ...
      mainPanel(
        ...
        if (!is.null(planner)) {
          tagList(
            awesomeCheckbox("planner_upload", paste0("Taak aanmaken in Plan '", planner$properties$title, "'"), value = FALSE, width = "100%"),
        ...
        if (!is.null(teams) && !is.null(planner)) { # teams_* project functions rely on planner, so require that too
          ...
          h5(HTML("Verbind eerst met MS Teams via <code>teams_connect()</code>."))
        },
        hr(),
  ...
  server <- function(input, output, session) {
    ...
    observeEvent(input$planner_upload, {
      if (input$planner_upload == TRUE) {
        updateRadioButtons(session = session, inputId = "files_teams_or_local", selected = "teams")
      }
    })
