      ...
      withProgress(message = "Aanmaken...", value = 0, {
        ...
        if (filetype %in% c(".Rmd", ".qmd")) {
          ...
          filecontent <- c(filecontent,
                           ...
                           "} else {",
                           paste0("  data_", project_id, " <- get_diver_data(date_range = c(start, stop),"),
                           paste0(strrep(" ", nchar(project_id)), "                          where = gl)"),
        ...
        if (filetype == ".R") {
          filecontent <- c(header_text,
                           "",
                           "library(certedata)",
                           paste0("data_", project_id, " <- get_diver_data(dates = c(start, stop),"),
                           paste0(strrep(" ", nchar(project_id)), "                        where = gl)"),
